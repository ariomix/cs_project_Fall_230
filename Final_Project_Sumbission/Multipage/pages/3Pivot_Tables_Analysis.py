...
def plotting(pivot_table,value_column, # plotting function. Main parameters are pivot_table and column values to plot
    ...
    match type: # match-case to match the coding needed for each chart type
        case "pie": # if a pie chart, then
            pivot_table[value_column].plot(kind=type, figsize=(8, 8),autopct='%1.1f%%') # pie chart is plotted with values as a percentage
            plt.title(title,fontdict= font_title) # title is given
            plt.ylabel(y_label, fontdict=font_labels) # y_label is given with the default value being none (so it is removed)
        case "bar": # if a bar chart, then
            plt.figure(figsize=(8,8)) # size of chart is plotted
            plt.bar(pivot_table.index,pivot_table[value_column],color = 'Blue') # bar type is plotted with all bars being blue
            if max != None and min != None: # Used ChatGPT. if max and min are given in the function, then
                plt.bar(pivot_table[pivot_table[value_column] == min].index,min, color='red') # min value is plotted as red
                plt.bar(pivot_table[pivot_table[value_column] == max].index,max, color='green') # max vlue is plotted as green
...
def main():
    ...
    df = reorder(filter_for_empty_and_useless(read_data_and_rename())) # data frame is created, filtered, and reordered (columns)

    match chosen: # based on the user's selection, we have code for each scenario ("Pie Graph", "Bar Chart", "Stacked Bar Chart")

        case "Pie Graph": # if Pie Graph, then
            pivot_vendors = df.pivot_table(values="OBJECTID", index="VENDOR", aggfunc="count") # Pivot Table of the number parking spots for each vendor type will be created
            st.pyplot(plotting(pivot_vendors,"OBJECTID", 'pie','Major Vendors')) # everything is plotted and plotted o streamlit

        case "Bar Chart": # if Bar Chart is selected, then
            pivot_streets = df.pivot_table(values="OBJECTID", index="STREET", aggfunc="count") # Number of parking lots by street
            pivot_streets = pivot_streets[pivot_streets["OBJECTID"] >= 100] # We are filtering for streets with at least 100 parking lots (since there are a lot of streets)
            pivot_streets = pivot_streets.sort_values(by="OBJECTID", ascending=True) # The major streets (>=100 in parking spots) are sorted in ascending order
            high, low = min_max(pivot_streets, "OBJECTID") # The street with the highest and the lowest parking spots are retrieved
            st.pyplot(plotting(pivot_streets, "OBJECTID",
                               'bar',
                               'Major Streets and amount of Parking Lots',
        ...
        case "Stacked Bar Chart": # if Stacked Bar Chart, then
